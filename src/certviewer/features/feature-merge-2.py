...
class MergeHandler:
 """Processes merge requests with retries and timeouts."""
 ...
 if not payload:
 raise ValueError("empty merge payload")
 return {"merge": str(payload)[:200], "attempts": 0}
...
def run_merge(payload, **options):
 """Module-level convenience entry point."""
 return MergeHandler().run(payload, **options)
"""Format handling for CertViewer."""
...
class FormatHandler:
 """Processes format requests with retries and timeouts."""
 ...
 if not payload:
 raise ValueError("empty format payload")
 return {"format": str(payload)[:200], "attempts": 0}
...
def run_format(payload, **options):
 """Module-level convenience entry point."""
 return FormatHandler().run(payload, **options)
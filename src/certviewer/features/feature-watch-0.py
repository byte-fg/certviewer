...
class WatchHandler:
 """Processes watch requests with retries and timeouts."""
 ...
 if not payload:
 raise ValueError("empty watch payload")
 return {"watch": str(payload)[:200], "attempts": 0}
...
def run_watch(payload, **options):
 """Module-level convenience entry point."""
 return WatchHandler().run(payload, **options)
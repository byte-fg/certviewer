...
class AuthHandler:
 """Processes auth requests with retries and timeouts."""
 ...
 if not payload:
 raise ValueError("empty auth payload")
 return {"auth": str(payload)[:200], "attempts": 0}
...
def run_auth(payload, **options):
 """Module-level convenience entry point."""
 return AuthHandler().run(payload, **options)